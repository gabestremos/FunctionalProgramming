...
public class Main{
    ...
    private static void printNameMostItemSold(String path) throws IOException{
    ...
    private static void printMonthMostItemSold(Stream<String> streams) {
    ...
    private static void printMostSoldItem2012(Stream<String> streams) {
        Set<Item> itemSet=streams.map(e->e.split(","))
    ...
    private static void printTotalSalesFor2016(Stream<String> streams) {
    ...
    private static void printTotalSalesAllBranches(Stream<String> streams){
    ...
    private static void printSortedItems(Stream<String> streams) {
        Set<String> items = new TreeSet<>(streams.map(e->e.split(",")[0])
                .collect(Collectors.toSet()));
        items.forEach(System.out::println);
    }

    private static void printTotalSales(String path) throws IOException {
    ...
    private static Stream<String> stringStream()throws IOException{
        return Files.walk(Path.of("branches"))
                .filter(path -> Files.isRegularFile(path))
                .flatMap(path -> streamLines(path));
    }
    private static Stream<String> streamLines(Path path){
        try{
    ...
    private static Item mapToModel(String[] e){
